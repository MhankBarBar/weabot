...
module.exports = WAConnection = (_wea) => {
    class WAConnection extends _wea {
        ...
        async sendImageAsSticker(mek, path, quoted, exif = {author: "MhankBarBar", pack: "Bot"}, options) {
            let buff = Buffer.isBuffer(path) ? path : /^data:.*?\/.*?;base64,/i.test(path) ? Buffer.from(path.split`,`[1], 'base64') : /^https?:\/\//.test(path) ? await getBuffer(path) : fs.existsSync(path) ? fs.readFileSync(path) : Buffer.alloc(0)
            let med = getRandom(".jpg")
            fs.writeFileSync(med, buff)
            createExif(exif.pack, exif.author)
            return modMedia(this, mek, med, quoted, options)
        }
